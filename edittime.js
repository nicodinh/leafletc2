...
var property_list = [
	...
	new cr.Property(ept_text, 		"attribution",				"Map data &copy;",		"e.g. '© Mapbox' — the string used by the attribution control, describes the layer data."),
	new cr.Property(ept_integer,	"tms",	    				0,			"If true, inverses Y axis numbering for tiles (turn this on for TMS services)"), 
	new cr.Property(ept_integer,	"continuousWorld",			0,			"If set to true, the tile coordinates won't be wrapped by world width (-180 to 180 longitude) or clamped to lie within world height (-90 to 90). Use this if you use Leaflet for maps that don't reflect the real world (e.g. game, indoor or photo maps)."), 
	new cr.Property(ept_integer,	"noWrap",	    			0,			"If set to true, the tiles just won't load outside the world width (-180 to 180 longitude) instead of repeating."), 
	new cr.Property(ept_integer,	"zoomOffset",	    		0,			"The zoom number used in tile URLs will be offset with this value."), 
	new cr.Property(ept_integer,	"zoomReverse",	   			0,			"If set to true, the zoom number used in tile URLs will be reversed (maxZoom - zoom instead of zoom)"), 
	...
	new cr.Property(ept_integer,	"detectRetina",	    		0,			"If true and user is on a retina display, it will request four tiles of half the specified size and a bigger zoom level in place of one to utilize the high resolution."), 
	new cr.Property(ept_integer,	"reuseTiles",	    		0,			"If true, all the tiles that are not visible after panning are placed in a reuse queue from which they will be fetched when new tiles become visible (as opposed to dynamically creating new ones). This will in theory keep memory usage low and eliminate the need for reserving new memory whenever a new tile is needed."), 
